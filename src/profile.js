...
export default function Profile() {
  ...
  useEffect(() => {
    if (createdVideo.video !== null)
      localforage.setItem(`Video${createdVideo.id}`, createdVideo.video);
  }, [createdVideo]);
  ...
  useEffect(() => {
    ...
    if (JSON.parse(window.localStorage.getItem("videoArray")).length > 6) {
      JSON.parse(window.localStorage.getItem("videoArray")).map((item) => {
        if (
          item.user.username === window.localStorage.getItem("currentUsername")
        ) {
  ...
  useEffect(() => {
    if (auxilliaryArray.length > 6) {
      window.localStorage.setItem(
        "videoArray",
        JSON.stringify(auxilliaryArray)
      );

      if (auxilliaryArray.length === array.length) setUpdated(true);
    }
  }, [auxilliaryArray]);
  ...
  return (
    <main id="profile-page-main">
      ...
      <article className="article-1">
            ...
            <input
              ...
              onKeyDown={(e) => {
                if (e.key === "Enter") {
                      ...
                      setTempArray((prev) => [
                        {
                          source: item.source,
                          video: item.video,
                          user: { image: item.user.image, username: username },
                          id: item.id,
                          key: item.key,
                        },
                        ...prev,
                  ...
                  setModifiedUsers([]);
                  JSON.parse(window.localStorage.getItem("usersArray")).forEach(
                    (item) => {
                      if (
                        item.username ===
                        window.localStorage.getItem("currentUsername")
        ...
        <Link id="log-out" to="/login" onClick={logOut}>
          Log out
        </Link>
      </article>
      <article className="article-2" ref={article2Ref}>
        <nav id="left-nav" ref={profileNavRef}>
          <CSSTransition
            in={renderProfileImage2}
            unmountOnExit
            timeout={300}
            classNames="nav-image"
          >
            <div
          ...
          <ul>
            <li>
              <i className="fa fa-phone"></i> +48 798 767 199
            </li>
            <li>
              <i className="fa fa-envelope"></i> szkulkerolf@gmail.com
            </li>
            <button id="chat">
        ...
        <div className="content-profile">
          <CSSTransition
            ...
            unmountOnExit
            timeout={300}
            classNames="change-image"
          >
            <ChangeImageForm
              setModifiedUsers={(value) => setModifiedUsers(value)}
              reference={changeRef}
          ...
          <CSSTransition
            ...
            unmountOnExit
            timeout={300}
            classNames="video-creator-transition"
          >
            <VideoProfile
              createdVideo={createdVideo}
              setCreatedVideo={(value) => setCreatedVideo(value)}
              setArray={(value) => setArray(value)}
              array={array}
              upload={(value) => setUpload(value)}
              uploaded={(value) => setUploaded(value)}
              loading={(value) => setLoading(value)}
            />
          </CSSTransition>
          <div id="my-videos">
            {JSON.parse(window.localStorage.getItem("videoArray")).length !==
              0 &&
              updated &&
              JSON.parse(window.localStorage.getItem("videoArray")).map(
                (item) => {
                  if (
                    item.user.username ===
                    window.localStorage.getItem("currentUsername")
                    ...
                    return (
                      <div className="my-videos-container">
                        ...
                        <div className="video-stats">
                          <span id="box-top"></span>
                          <span id="box-left"></span>
                          <span id="box-right"></span>
                          <span id="box-bottom"></span>
                          <p>
                            <i className="fa-solid fa-message"></i>Lorem ipsum
                            dolor sit amet. Ut harum voluptates et iure fuga vel
                            eveniet cumque qui fugiat iusto sit libero sint ea
                            voluptates autem. Aut velit aliquid est cupiditate
                            quia eum delectus maiores sed sint sint ea sequi
                            distinctio et odit architecto ex quaerat cumque?
                          </p>
                          <p>
                            <i className="fa fa-heart"></i>Et ipsa doloremque
                            rem eaque nihil ut corrupti repellendus et nihil
                            expedita qui asperiores veritatis quo vero commodi.
                            Eum incidunt esse est iure exercitationem ea tenetur
                            velit nam molestias autem et distinctio ipsum. Eos
                            doloremque debitis est rerum ullam rem nesciunt
                            natus nam explicabo ullam. Sed deserunt quia est
                            omnis Quis aut maiores dolorem At doloremque enim et
                            minima dolorem.{" "}
                          </p>
                          <p>
                            <i className="fa fa-share"></i>Vel architecto cumque
                            et temporibus beatae qui internos aliquid est sequi
                            provident ex dolorum velit in voluptas placeat sed
                            ullam quibusdam. Quo natus nihil sit nostrum illum
                            et nihil aperiam et aperiam facilis ab alias maxime.{" "}
                          </p>
                        </div>
                      </div>
                    );
