...
export default function VideoProfile(props) {
  ...
  useEffect(() => {
    if (selectedVideo) {
      props.setCreatedVideo({
        ...
        user: {
          image: JSON.parse(window.localStorage.getItem("usersArray")).map(
            (item) => {
              if (
                item.username === window.localStorage.getItem("currentUsername")
              ) {
  ...
  return (
    <section id="video-creator-section">
      <button onClick={() => props.upload(false)} id="close-upload">
      ...
      <div id="creator">
        ...
        <div id="video-options-buttons">
          ...
          >
            <button></button>
            <p className="paragraph">Disallow comments</p>
            <i className="fa fa-message"></i>
          </span>
          <span onClick={toggleButton} className="allow">
            <button></button>
            <p className="paragraph">Allow shares</p>
            <i className="fa fa-share"></i>
          </span>
          <span onClick={toggleButton} className="allow">
            <button></button>
            <p className="paragraph">Allow stitches</p>
            <i className="fa fa-adjust"></i>
          </span>
          <span onClick={toggleButton} className="allow">
            <button></button>
            <p className="paragraph">Allow downloads</p>
            <i className="fa fa-download"></i>
          </span>
        </div>
        <button id="finish-upload" onClick={() => uploadVideo()}>
          Upload
        </button>
      </div>
      <div id="live-view">
        <div className="black-background-profile">
          ...
          >
            ...
            {!videoLink && <span> Select a video file</span>}
          </button>
          {videoLink && (
            <video loop autoPlay width="100%" ref={profileVideoRef}>
